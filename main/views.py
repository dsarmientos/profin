...
def result(request):
    min_size = int(request.GET['min']) if request.GET['min'] else 1500
    max_size = int(request.GET['max']) if request.GET['max'] else 18000
    img_name = request.GET['img_name']

    img = mahotas.imread(settings.MEDIA_ROOT+img_name)
    filename = img_name.split(".")[0]
    f, blue_component, combined = imgsegment.segment(img, filename, min_size = min_size, max_size = max_size)
    ncells, _ = imgsegment.count(img, f, combined, blue_component, filename)

    return render(request, 'result.html', {"smg_img":'%s-output-%dcells.jpg' % (filename, ncells), "ncells":ncells})
